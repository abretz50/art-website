...
- [x] Per feedback, "At a Glance" now only links to pages that actually exist on
      this site — My Music, My Poetry, My Vocals, and Promotional Materials (the
      Press Kit). The Rowan University and Philadelphia Symphonic Chorus links
      were removed since neither is a page on this site.
- Reference: Art confirmed the Philadelphia ensemble's Instagram is
  `instagram.com/philadelphiasymphonicchoir` (note: "Choir," not "Chorus" as
  written in the bio text) — not currently used anywhere, but here if a social
  link for it is ever wanted (e.g. added to the homepage's "Learn More" chips).
